...
// Računa rastojanje između dve tačke (x1, y1) i (x2, y2)
double rastojanje(double x1, double y1, double x2, double y2) {
    return sqrt((x2 - x1)*(x2 - x1) + (y2 - y1)*(y2 - y1));
}

// Računa površinu trougla po Heronovoj formuli
...
int main() {
    ...
    const double EPS = 1e-9; // grcko epsilon, prag tolerancije npr -> 1 i 0.999999999

    if (abs((P1 + P2 + P3) - P) < EPS) {
        cout << "da\n";
    } else {
        cout << "ne\n";
    }
